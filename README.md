...
Playwright tests are in `tests/e2e` and use `playwright.config.ts`.
When you run e2e, Playwright automatically starts the app server on `127.0.0.1:3000`.
...
- If `3000` is busy, run with a different port:

```bash
PORT=3100 pnpm test:e2e
```

- HTML report is generated by Playwright. Open it with:
